...
async def _api_async(method, params):
...
async def payment_upgrade(client, cb):
    ...
    if not getattr(client, "is_main_bot", False):
        await cb.message.reply_text("💎 Premium purchases are handled by the main AniToon bot.")
...
async def payment_buy(client, cb):
    if not getattr(client, "is_main_bot", False):
        await cb.answer("Payments are available on the main bot.", show_alert=True)
        raise StopPropagation
    ...
    try:
        await _api_async("sendInvoice", {
            "chat_id": cb.from_user.id,
            "title": f"AniToon {plan.name}"[:32],
            "description": f"{plan.name} for 30 days. Daily limit {humanbytes(plan.daily_limit)}."[:255],
            "payload": payload,
            "currency": "XTR",
...
async def payment_precheckout(client, update, users, chats):
    if not getattr(client, "is_main_bot", False) or not isinstance(update, UpdateBotPrecheckoutQuery):
        return
    try:
        payload = update.payload.decode() if isinstance(update.payload, bytes) else str(update.payload)
        parts = payload.split("|")
        if len(parts) != 4 or parts[0] != "anitoon" or parts[1] not in PLANS:
            raise ValueError("Invalid payment payload")
        plan = get_plan(parts[1])
        if int(update.user_id) != int(parts[3]) or update.currency != "XTR" or int(update.total_amount) != plan.stars:
            raise ValueError("Invalid payment")
        await client.invoke(SetBotPrecheckoutResults(query_id=update.query_id, success=True))
    except Exception as e:
        try:
            await client.invoke(SetBotPrecheckoutResults(query_id=update.query_id, success=False, error=str(e)[:200]))
        except Exception:
            pass


def _payment_filter(_, __, message):
...
async def payment_success(client, message):
    ...
    try:
        ...
        if int(parts[3]) != message.from_user.id or payment.currency != "XTR" or int(payment.total_amount) != plan.stars:
        ...
        if await db.record_payment(message.from_user.id, target_bot, plan.key, payment.total_amount, charge):
            ...
            await message.reply_text("ℹ️ This Stars payment was already processed.")
    except Exception as e:
