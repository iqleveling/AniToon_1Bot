...
It only validates the update, enforces ForceSub for the main bot, and hands the
file to the deferred job pipeline. Multiple files are accepted automatically;
the invisible FIFO queue keeps them registered without exposing queue buttons
or commands.
"""
...
async def route_file_to_pipeline(client, message):
    """Fast intake gate; every valid file enters the automatic queue."""
    ...
    queue_count = 0
    try:
        queue_count = len(await jobs.get_user_jobs(int(user_id)))
    except Exception:
        pass

    log.info(
        "Routing file into automatic queue: bot_id=%s user_id=%s message_id=%s type=%s name=%s size=%s user_jobs=%s",
        getattr(client, "bot_id", 0), user_id, getattr(message, "id", "unknown"),
        media_type, file_name, humanbytes(file_size), queue_count + 1,
    )
